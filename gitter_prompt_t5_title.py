...
def read_arguments():
	...
	parser.add_argument("--output_dir", default="./model_gitter_prompt", type=str, required=False,
...
def main(args):
	...
	plm = T5ForConditionalGeneration.from_pretrained("t5-small", config=model_config)
	...
	promptTemplate = SoftTemplate(model=plm, tokenizer=tokenizer,
										  text='summarize: {"placeholder":"text_a"} {"mask"}', initialize_from_vocab=True,
										   num_tokens=20) #  'Text: {"placeholder":"text_a"} Summarization: {"mask"} '
	...
	if args.do_train:
		...
		random.seed(1024)
		train_examples = random.sample(train_examples, len(train_examples))
