import torch
import numpy as np
from transformers import RobertaTokenizer,BertTokenizer, RobertaModel, BertModel
import pandas as pd
...
def build_model(name):
    tokenizer = RobertaTokenizer.from_pretrained(name, use_fast=True)
...
def main():
    ...
    df = pd.read_csv("data/gitter.csv", header=None, sep='\t')
    code_list = df[2].tolist()
    print(len(code_list))
    ...
    import pickle
    f = open('models/code_vector_whitening.pkl', 'wb')
    pickle.dump(vecs_func_body, f)
    f.close()
    f = open('models/kernel.pkl', 'wb')
    pickle.dump(kernel, f)
    f.close()
    f = open('models/bias.pkl', 'wb')
    pickle.dump(bias, f)
    f.close()
...
def main2():
    ...
    print(vecs_func_body.shape)
    import pickle
    f = open('models/code_vector_whitening_issues.pkl', 'wb')
    pickle.dump(vecs_func_body, f)
    f.close()
    f = open('models/kernel_issues.pkl', 'wb')
    pickle.dump(kernel, f)
    f.close()
    f = open('models/bias_issues.pkl', 'wb')
    pickle.dump(bias, f)
    f.close()
