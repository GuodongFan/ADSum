...
import sys
print(sys.path)
sys.path.append('/data1/fgd/workplace/DSum/')
import torch
import random
import logging
import argparse
...
def read_arguments():
	...
	parser.add_argument("--output_dir", default="./RQ2/model_gitter_ft", type=str, required=False,
...
def main(args):
	...
	file_path = args.output_dir + '_' + str(train_num_sample) +  '_' + model_name + '_' + test_file
...
def calculate_bleu(file_name, args, tokenizer, device, model, file_postfix=None, is_test=False, dev_dataset=None,
	...
	metrics_dict = compute_metrics(hypothesis=os.path.join(args.output_dir, file_prefix + "{}.output".format(file_postfix)),
								   references=[os.path.join(args.output_dir, file_prefix + "{}.gold".format(file_postfix))], no_skipthoughts=True,
								   no_glove=True)
