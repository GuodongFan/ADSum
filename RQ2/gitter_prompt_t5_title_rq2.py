...
def read_arguments():
	...
	parser.add_argument("--output_dir", default="./RQ2/model_gitter_pt", type=str, required=False,
...
def main(args):
	...
	# test_file = "ts"
	#train_filename = args.data_dir + "/" + "/gitter/ethereum.csv"	# train
	#dev_filename = args.data_dir + "/" + "/gitter/appium.csv"	# valid
	#test_filename = args.data_dir + "/" + "/gitter/ts.csv"	# test

	test_file = "ethereum"
	# data path
	train_filename = args.data_dir + "/" + "/gitter/appium.csv"	# train
	dev_filename = args.data_dir + "/" + "/gitter/ts.csv"	# valid
	test_filename = args.data_dir + "/" + "/gitter/ethereum.csv"	# test
	...
	file_path = args.output_dir + '_' + str(num_tokens) + '_' + str(train_num_sample) +  '_' + model_name + '_' + test_file
	...
	promptTemplate = SoftTemplate(model=plm, tokenizer=tokenizer,
										  text='summarization: {"placeholder":"text_a"} {"mask"} ', initialize_from_vocab=True,
	...
	if args.do_train:
		...
		random.seed(1024)
		train_examples = random.sample(train_examples, min(train_num_sample, len(train_examples)))
...
def calculate_bleu(file_name, args, tokenizer, device, model, promptTemplate, WrapperClass, output_file_name=None, is_test=False, dev_dataloader=None,
	...
	metrics_dict = compute_metrics(hypothesis=os.path.join(args.output_dir, file_prefix + "{}.output".format(output_file_name)),
								   references=[os.path.join(args.output_dir, file_prefix + "{}.gold".format(output_file_name))], no_skipthoughts=True,
								   no_glove=True)

	#(goldMap, predictionMap) = bleu.computeMaps(predictions,
	#											)
	#this_bleu = round(bleu.bleuFromMaps(goldMap, predictionMap)[0], 2)
	this_bleu = metrics_dict['Bleu_4']
