...
parser.add_argument('prefix',
                    default="./models/example-train",
                    help="Start of names for files produced.")

parser.add_argument('--suffix', default='', help="Suffix of the output file.")

# Data arguments
parser.add_argument('--train',
                    help="Training files, e.g. train/*annotation.txt")  # , default='../data/retrain/train/*annotation.txt'
parser.add_argument('--dev',
                    help="Development files, e.g. dev/*annotation.txt")  # , default='../data/retrain/dev/*annotation.txt'
parser.add_argument('--test', help="Test files, e.g. test/*annotation.txt",
                    default='../data/retrain/test/test_Typescript/*ascii*')  # , default='../data/test_disentangled/*ascii*'
parser.add_argument('--test-start', type=int, help="The line to start making predictions from in each test file.",
                    default=0)  # , default=0
parser.add_argument('--test-end', type=int, help="The line to stop making predictions on in each test files.",
                    default=300000)  # , default=105930
parser.add_argument('--model', help="A file containing a trained model",
                    default='./src/disentanglement/example-train.dy.model')  #
...
parser.add_argument('--nonlin',
                    ...
                    default='softsign', help="Non-linearity type.")

# Inference arguments
parser.add_argument('--max-dist', default=11, type=int,
                    help="Maximum number of messages to consider when forming a link (count includes the current message).") # 101改成21试试
...
parser.add_argument('--learning-decay-rate', default=0.103, type=float,
...
def lines_to_info(text_ascii):
    ...
    for line_no, line in enumerate(text_ascii):
        ...
        if len(line) > 0 and line[0].startswith("[") and line[0][1].isdigit() and len(line) >= 3:
            user = line[1][1:-1]
...
def read_data(filenames, is_test=False):
    ...
    for filename in filenames:
        ...
        #if GITTER:
            # 20220410 训练数据没有T 还是一劳永逸的解决这个问题吧
        #    text_ascii = [l.split(" ",1)[0]+'T'+l.split(" ",1)[1] for l in open(name+".ascii.txt", encoding='utf8')]
        #    text_ascii = [l.strip().split() for l in text_ascii]
        #else:
...
if args.train:
    ...
    for project_name in project_name_list:
        ...
        for train_data in train:
            project_from = train_data[0]
            # 什么意思？不等于的时候放入train_dev_list 感觉和k折交叉差不多 和后面的N_FOLD重复
            ...
            for epoch in range(EPOCHS):
                ...
                for instance in train_list:
                    ...
                    if step % args.report_freq == 0:
                        ...
                        print("epoch {} loss {:.3f} train_acc {:.3f} dev_acc {:.3f} dev_match {} dev_total {}".format(epoch, loss / loss_steps, tacc, dacc,
                                                                                   dev_match, dev_total))
                        #print("{} tl {:.3f} ta {:.3f} da {:.3f} match {} total {}".format(epoch, loss / loss_steps, tacc, dacc,
                        ...
                        if prev_best is None or prev_best[0] < dacc:
                            ...
                            print('save model')
                            model.model.save(args.prefix + f".dy_{str(i)}.model")  # Former version: ".dy.model"\
...
if prev_best is not None or args.model:
    location = args.model
    if location is None:
        location = args.prefix + ".dy.model"  # Former version: ".dy.model"
        print('load:', location)
...
for instance in test:
    ...
    with open(instance[0]+args.suffix, 'a') as f:
        f.write("{}:{} {} -".format(instance[0], instance[1], instance[1] - prediction))
        f.write('\n')
