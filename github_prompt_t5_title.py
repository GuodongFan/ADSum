...
def read_arguments():
	...
	parser.add_argument("--output_dir", default="./model_github_pt", type=str, required=False,
...
def main(args):
	...
	# read model --------------------------------------------------------------
	small = "t5-small" # t5-base
	model_config = T5Config.from_pretrained(small)
	#plm = T5ForConditionalGeneration.from_pretrained("./model/checkpoint-best-bleu/", config=model_config)
	plm = T5ForConditionalGeneration.from_pretrained(small, config=model_config)
	tokenizer = T5Tokenizer.from_pretrained(small)
